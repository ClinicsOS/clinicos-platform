 ...
 */
export const getGrowthStats = asyncHandler(async (_req: Request, res: Response) => {
  const now = new Date();
 ...
 */
export const getPlansDistribution = asyncHandler(async (_req: Request, res: Response) => {
  const rows = await Clinic.aggregate([
    { $match: { status: "active" } },
 ...
 */
export const getRevenueStats = asyncHandler(async (_req: Request, res: Response) => {
  const now = new Date();
  const twelveMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 11, 1);
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const year = d.getFullYear();
    const month = d.getMonth() + 1;
    const monthRows = rows.filter((r) => r._id.year === year && r._id.month === month);
...
export const listClinics = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (expiring === "3" || expiring === "7") {
    const now = new Date();
    const end = new Date(now.getTime() + Number(expiring) * 24 * 60 * 60 * 1000);
  ...
  const patientMap = new Map(patientCounts.map((p) => [p._id.toString(), p.count]));
...
export const getClinicDetails = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [users, patientCount, appointmentCount, invoices, subscriptionHistory] =
    await Promise.all([
...
export const changeClinicPlan = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Invalid clinic id" });
  }
  ...
  if (parsed.data.extendDays && parsed.data.extendDays > 0) {
    ...
    const base = clinic.planExpiresAt > new Date() ? clinic.planExpiresAt : new Date();
...
export const extendClinicPlan = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Invalid clinic id" });
  }
...
export const changeClinicStatus = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Invalid clinic id" });
  }
  ...
  await logActivity({
    ...
    targetId: clinic._id as Types.ObjectId,
    targetLabel: clinic.name,
    details: { oldStatus, newStatus: clinic.status, reason: parsed.data.reason },
  });
...
export const deleteClinic = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { confirmText, reason } = req.body as { confirmText?: string; reason?: string };
...
export const exportClinic = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [clinic, users, patients, appointments, invoices, subscriptionRequests] =
    await Promise.all([
 ...
 */
export const impersonateClinic = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Invalid clinic id" });
  }
  ...
  const owner = await User.findOne({ clinicId: id, role: "owner", isActive: true });
  if (!owner) {
  ...
  const token = jwt.sign({ userId: String(owner._id) }, secret, { expiresIn: "1h" });
...
export const approveUpgradeRequest = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
  ...
  if (!clinic) return res.status(404).json({ message: "Associated clinic not found" });
...
export const rejectUpgradeRequest = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Invalid request id" });
  }
 ...
 */
export const verifyUserEmailManually = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Invalid user id" });
  }
...
export const sendEmailToClinic = asyncHandler(async (req: Request, res: Response) => {
  ...
  await sendAdminCustomEmail(owner.email, owner.name, parsed.data.subject, parsed.data.body);
...
export const changeAdminPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.json({
    message: "New hash generated. Update ADMIN_PASSWORD_HASH env var and redeploy to apply.",
    newHash,
    instructions:
